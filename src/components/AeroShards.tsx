...
const layoutVector = (placement: number): [number, number, number, number] =>
  [0, 1, 2, 3].map(index => (index === placement ? 1 : 0)) as [number, number, number, number];

const createFormation = (flow: number) => ({ weights: layoutVector(flow), velocity: [0, 0, 0, 0] as [number, number, number, number] });
...
const advanceHold = (hold: { pointerId: number | null; elapsed: number; amount: number; velocity: number; phase: number }, elapsed: number, disabled: boolean) => {
  if (disabled) {
    hold.pointerId = null;
    hold.elapsed = 0;
    hold.amount = 0;
  ...
  const targetVal = engaging ? 1 : 0;
  const response = engaging ? 3.8 : 3.2;
  const decay = Math.exp(-response * step);
  const offset = hold.amount - targetVal;
  const momentum = hold.velocity + response * offset;
  hold.amount = targetVal + (offset + momentum * step) * decay;
  hold.velocity = (hold.velocity - response * momentum * step) * decay;
  if (Math.abs(hold.amount - targetVal) < 0.0001 && Math.abs(hold.velocity) < 0.001) {
    hold.amount = targetVal;
    hold.velocity = 0;
  }
  if (hold.amount > 0) hold.phase += elapsed * (0.35 + hold.amount * 0.5);
};

const resetPointerMotion = (pointer: { velocity: [number, number]; presenceVelocity: number }) => {
  pointer.velocity[0] = 0;
  pointer.velocity[1] = 0;
  pointer.presenceVelocity = 0;
};

const advancePointer = (pointer: { position: [number, number]; raw: [number, number]; velocity: [number, number]; active: number; presence: number; presenceVelocity: number }, elapsed: number) => {
  if (elapsed <= 0) return;
...
const createRipples = () => Array.from({ length: 4 }, () => ({ origin: [0.5, 0.5] as [number, number], age: 0, duration: 0, strength: 0 }));

const startRipple = (ripples: Array<{ origin: [number, number]; age: number; duration: number; strength: number }>, origin: [number, number], aspect: number, strength = 1) => {
...
const advanceRipples = (ripples: Array<{ origin: [number, number]; age: number; duration: number; strength: number }>, elapsed: number, disabled: boolean) => {
...
for (let sampleIdx = 0; sampleIdx < 6; sampleIdx += 1) {
  const peak = Math.max(...ASCII_SHAPES.map(shape => shape[sampleIdx]));
  for (const shape of ASCII_SHAPES) shape[sampleIdx] /= Math.max(peak, 0.001);
}
...
const resolveBloomSize = (size: readonly [number, number] | [number, number], qualityLevel = 0): [number, number] => {
...
const createRenderGraph = (gpu: any, outputSize: readonly [number, number] | [number, number], bloomSize = resolveBloomSize(outputSize)) => {
...
const configureStyle = (graph: any, settings: any, outputSize: readonly [number, number] | [number, number], cssSize: [number, number]) => {
...
export default function AeroShards({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        setReady(false);
        const resolvedQualityPreset = resolveQuality(canvas);
        const preset = QUALITY_PRESETS[resolvedQualityPreset] || QUALITY_PRESETS.medium;
        gpu = await init({ powerPreference: 'low-power' });
        if (disposed) return gpu.dispose();
        unsubscribeGpuError = gpu.onError(reportFailure);

        const gpuNavigator = (navigator as any).gpu;
        if (!gpuNavigator) {
          throw new Error('WebGPU is not supported in this browser.');
        }
        const outputFormat = gpuNavigator.getPreferredCanvasFormat();
        ...
        const setRuntimeQuality = (nextLevel: number, now: number, frameState: { interval: number; continuous: boolean }) => {
